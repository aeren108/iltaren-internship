...
#define MAX_DS_GROUP_DIST 250
#define MIN_DS_GROUP_SIZE 3
#define MAX_STAGGER_PERIOD 250
...
Vector<Vector<int, MAX_GROUP>, MAX_GROUP> sliding_groups;
Vector<Vector<int, MAX_PULSE>, MAX_GROUP> all_sliding_groups;

Vector<Vector<int, MAX_GROUP>, MAX_GROUP> dwellnswitch_groups;
Vector<Vector<int, MAX_PULSE>, MAX_GROUP> all_dwellnswitch_groups;
...
double average_increasing_rate(Vector<int, MAX_PULSE>& group) {
	...
	for (int i = 2; i < group.size(); ++i)
		sum += (toa[group[i]] - toa[group[i - 1]]) - (toa[group[i - 1]] - toa[group[i - 2]]);
	...
	for (int i = 0; i < toa.size(); ++i) {
		...
		for (int j = i + 1; j < temparr.size(); ++j) {
			...
			if (length > maxlen) {
				maxlen = length;
				visited = visited_tmp;
				period = toa[visited[1]] - toa[visited[0]];
			}
		}
		...
		for (int j = 0; j < visited.size(); ++j) {
			if (DEBUG) std::cout << toa[visited[j]] << ", ";
...
void extract_sliding_groups(Vector<Vector<int, MAX_PULSE>, MAX_GROUP>& groups, Vector<Vector<int, MAX_GROUP>, MAX_GROUP>& groups2) {
	...
	for (int i = 0; i < groups.size(); ++i) { 
		...
		for (int j = i + 1; j < groups.size(); ++j) {
			...
			int expected_diff = toa[groups[j][0]] - toa[groups[i][groups[i].size() - 1]];
			...
			for (int k = j + 1; k < groups.size(); ++k) {
				...
				int diff = toa[groups[k][0]] - toa[groups[j][groups[j].size() - 1]];
	...
	for (int i = 0; i < groups2.size(); ++i) {
		...
		for (int j = 0; j < groups2[i].size(); ++j) {
			for (int k = 0; k < groups[groups2[i][j]].size(); ++k) {
				std::cout << toa[groups[groups2[i][j]][k]] << ", ";
			}
			std::cout << " | ";
		}
...
void extract_dwellnswitch_groups(Vector<Vector<int, MAX_PULSE>, MAX_GROUP>& groups, Vector<Vector<int, MAX_GROUP>, MAX_GROUP>& groups2) {
		...
		for (int j = i + 1; j < temparr.size(); ++j) {
			...
			for (int k = j + 1; k < temparr.size(); ++k) {
				...
				if (temparr[k] > expected_time) break; //There is no possible value for rest of the array
					...
					length++;
					visited_tmp.push_back(k);
					if (visited_tmp.size() >= MIN_DS_GROUP_SIZE) {
						periods.push_back(toa[visited_tmp[1]] - toa[visited_tmp[0]]);
						groups.push_back(visited_tmp);
					}
				}
			}
	...
	for (int i = 0; i < groups.size(); ++i) {
		...
		int period = toa[groups[i][1]] - toa[groups[i][0]];
		int maxlen = 0;
		...
		for (int j = i + 1; j < groups.size(); ++j) {
			...
			int dist = toa[groups[j][0]] - toa[groups[i][groups[i].size() - 1]];
			int prevdist = dist * 3;
			int prevk = j;
			int length = 1;
			...
			if (dist > MAX_DS_GROUP_DIST || dist < 0 || dist != periods[j] || dist > prevdist * 2) continue;

			for (int k = j + 1; k < groups.size(); ++k) {
				if (group_marks[k] ||
					groups[i].size() != groups[k].size()) continue;

				dist = toa[groups[k][0]] - toa[groups[prevk][groups[prevk].size() - 1]];

				if (dist > MAX_DS_GROUP_DIST || dist < 0) continue;
...
void extract_stable_groups(Vector<Vector<int, MAX_PULSE>, MAX_GROUP>& groups, Vector<int, MAX_PULSE>& periods) {
	for (int i = 0; i < toa.size(); ++i) { //Find a pattern for i'th element
		if (marks[i]) continue;
...
int main() {
	...
	for (int i = 0; i < toa.size(); ++i) marks.push_back(false);

	//preprocess_times();

	extract_sliding_groups(all_sliding_groups, sliding_groups);

	for (int i = 0; i < sliding_groups.size(); ++i) {
		for (int j = 0; j < sliding_groups[i].size(); ++j) {
			for (int k = 0; k < all_sliding_groups[sliding_groups[i][j]].size(); ++k) {
				marks[all_sliding_groups[sliding_groups[i][j]][k]] = true;
			}
		}
	}

	extract_staggering_groups(all_staggering_groups, staggering_groups);

	for (int i = 0; i < staggering_groups.size(); ++i) {
		for (int j = 0; j < staggering_groups[i].size(); ++j) {
			for (int k = 0; k < all_staggering_groups[staggering_groups[i][j]].size(); ++k) {
				marks[all_staggering_groups[staggering_groups[i][j]][k]] = true;
			}
		}
	}

	extract_dwellnswitch_groups(all_dwellnswitch_groups, dwellnswitch_groups);

	for (int i = 0; i < dwellnswitch_groups.size(); ++i) {
		for (int j = 0; j < dwellnswitch_groups[i].size(); ++j) {
			for (int k = 0; k < all_dwellnswitch_groups[dwellnswitch_groups[i][j]].size(); ++k) {
				marks[all_dwellnswitch_groups[dwellnswitch_groups[i][j]][k]] = true;
			}
		}
	}
	...
	extract_stable_groups(stable_groups, stable_periods);

	std::cout << "BUS COUNT: " << stable_periods.size() << std::endl;
	std::cout << "PERIODS: " << stable_periods << std::endl;
	std::cout << "GROUPS: " << std::endl;

	for (int i = 0; i < stable_groups.size(); ++i) {
		for (int j = 0; j < stable_groups[i].size(); ++j) 
			std::cout << toa[stable_groups[i][j]] << ", ";
		std::cout << std::endl;
	}
